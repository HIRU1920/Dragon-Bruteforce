import optparse
import zipfile
import rarfile  # type: ignore # need unrar tools
import itertools
import string
import os
import crypt
...
import time
import term  # type: ignore # py-term
...
class Dragon():
    ...
    def banner(self):
        ...
        bannerfont = "Crotah " if os.path.exists('/usr/share/figlet/epic.flf') else "banner"
        banner = pyfiglet.figlet_format("Dragon", font=bannerfont).replace("\n", "\n\t\t", 7)
		
        cprint("\r\n\t" + "@" * 61, "red", end="")
        cprint("\n\t\t" + banner + "\t\tAuthor : Hir Patel", "green", attrs=['bold'])
        cprint("\t" + "@" * 61 + "\n", "red")
